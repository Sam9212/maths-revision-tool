...
fn debug_fetch_all() -> Vec<User> {
    // filter_map is a special iterator pattern that applies a mutating function to
    // all elements of an iterator and filters out any None values from the resulting
    // `Option<T>` that is returned in each map pass.
